...
function generateTicket() {
    var cols, finalTicket, flag = true, colPlaceholder = [], val = 0, finalTicket2 = Array(2);;
    while (flag) {
        cols = Array(9).fill(1);
        finalTicket = Array(3);
        if (val == 0)
            finalTicket2[0] = Array(3).fill(0);
        finalTicket2[1] = Array(3).fill(0);
        ...
        for (i = 0; i < finalTicket2[val].length; i++) {
            finalTicket2[val][i] = finalTicket[i];
        }
        //finalTicket2[val] = finalTicket;
        if (val == 1) {
            flag = false;
        }
        val = val + 1;
        cols.length = 0;
        finalTicket.length = 0;
        colPlaceholder.length = 0;
    }
    return finalTicket2;
}
...
module.exports = { generateTicket, getDrawSequence };